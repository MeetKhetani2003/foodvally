...
async function fetchWikiImage(query) {
    const url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=pageimages&generator=search&gsrsearch=" + encodeURIComponent(query) + "&pithumbsize=600";
    try {
...
async function run() {
    const updatedData = {};
    for (const [category, items] of Object.entries(rawMenuData)) {
        console.log("Fetching " + category + "...");
        updatedData[category] = [];
        for (const item of items) {
            let img = await fetchWikiImage("vegetarian " + item);
            if (!img) {
                img = await fetchWikiImage(item);
            }
            
            const productSlug = item.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
            if (!img) {
                img = "https://loremflickr.com/600/400/indian,food," + encodeURIComponent(item.replace(/ /g, ''));
            }
    ...
    const fileContent = "export const allMenuProductsWithGoogleLinks = " + JSON.stringify(Object.values(updatedData).flat(), null, 2) + ";";
    fs.writeFileSync('src/utils/menuDataWithImages.ts', fileContent);
    console.log('Done!');
}
